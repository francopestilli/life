...
%
%  - 1 - Set up a LiFE structure, identified as 'fe' (fascicle evaluation) in
...
%  voxels.
%
%  - 2 - Fit the LiFE model to compute the weights associated to each fascicle
...
%  
%  - 3 - Compare two different connectome models. This demo will show how to
...
%% Initialize the LiFE model structure, 'fe' in the code below. 
...
%% Extract the RMSE of the model on the second data set. 
...
%% Compute the strength-of-evidence (S) and the Earth Movers Distance.
% Compare the RMSE of the two models using the Stregth-of-evidence and the
% Earth Movers Distance.
se = feComputeEvidence(prob.rmse,det.rmse);

% Show the strength of evidence in favor of Probabilistic versus
% Deterministic tractography. Plot the distributions of resampled mean RMSE
% used to compute the strength of evidence (S).
fh(5) = distributionPlotStrengthOfEvidence(se);

% Show the RMSE distributions for Probabilistic and Deterministic
% tractography. Compare the distributions using the Earth Movers Distance.
