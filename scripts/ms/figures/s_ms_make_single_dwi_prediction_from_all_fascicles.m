function s_ms_make_single_dwi_prediction_from_all_fascicles(slices,savefigures,figuredir)
%
% This script builds and fits a LiFE model for few fascicles then generates 
% a volume of predicted DWI signal from each fascicle and combines these
% volumes into a single nifti file.
%
% Franco
global gPlotBox
if notDefined('figuredir'), figuredir  = '~/Dropbox/ms_dwi_fascicle_prediction_figures/signal';end
if notDefined('mapsdir'),  mapsdir   = '~/Dropbox/fascicle_maps/';end
...
%---------------------------------------%
function makeAllMoviesLocal(fe,tag,figuredir,movieFileName, slice, nframes,savemovie,  dwiNifti)
disp('making movies...')
...
if strcmpi(tag,'_arcuate_cst')
  ...
  % Open a video object
  if savemovie
    vidObj = VideoWriter(fullfile(figuredir,  [movieFileName, '_original_dw_signal_slice',num2str(slice),tag]));
    vidObj.FrameRate = 10;
    vidObj.Quality = 90;
    open(vidObj);
  end
  ...
  % Open a video object
  if savemovie
    vidObj = VideoWriter(fullfile(figuredir,  [movieFileName, '_arcuate_dw_signal_slice',num2str(slice),tag]));
    vidObj.FrameRate = 10;
    vidObj.Quality = 90;
    open(vidObj);
  end
...
% Open a video object
if savemovie
  vidObj = VideoWriter(fullfile(figuredir,  [movieFileName, '_predicted_dw_signal_slice',num2str(slice),tag]));
  vidObj.FrameRate = 10;
  vidObj.Quality = 90;
  open(vidObj);
end
...
% Open a video object
if savemovie
  vidObj = VideoWriter(fullfile(figuredir,  [movieFileName, '_residual_dw_signal_slice',num2str(slice),tag]));
  vidObj.FrameRate = 10;
  vidObj.Quality = 90;
  open(vidObj);
end
...
colorbar
% Save a figure
feSavefig(gcf,'figName',['arcuate_rmse_ratio_slice',num2str(slice),tag,'.jpg'],'figDir',figuredir,'figType','jpg')
feSavefig(gcf,'figName',['arcuate_rmse_ratio_slice',num2str(slice),tag,'.png'],'figDir',figuredir,'figType','png')
feSavefig(gcf,'figName',['arcuate_rmse_ratio_slice',num2str(slice),tag,'.eps'],'figDir',figuredir,'figType','eps')
...
colorbar 
% Save a figure
feSavefig(gcf,'figName',['arcuate_rmse_slice',num2str(slice),tag,'.jpg'],'figDir',figuredir,'figType','jpg')
feSavefig(gcf,'figName',['arcuate_rmse_slice',num2str(slice),tag,'.png'],'figDir',figuredir,'figType','png')
feSavefig(gcf,'figName',['arcuate_rmse_slice',num2str(slice),tag,'.eps'],'figDir',figuredir,'figType','eps')
