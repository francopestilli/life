function s_life_fg_afq_stt_vs_tend(runType,fitType,save)
% function s_life_fg_afq_stt_vs_tend
%
...
if notDefined('runType');         runType = 'common';end % Run life on common voxels defined as the intersection between the two fibers. 
...
for fg_i = 1:length(which_fascicles)
  ...
  switch runType
    case {'independently', 'i','ind'} % On independent voxels for TEND and STT
      ...
      [stt tend] = runLifeBothAlgos(tend,stt,dwi,nifti,fg_i);
    ...
    case {'same','common','s','c'}    % on the same, common voxels between the two fiber groups
      ...
      stt.runType  = runType;
      tend.runType = runType;
      [stt tend] = runLifeCommonVoxels(tend,stt,dwi,nifti,fg_i);
...
% runLifeCommonVoxels %
%%%%%%%%%%%%%%%%%%%%%%%
function [stt tend] = runLifeCommonVoxels(tend,stt,dwi,nifti,fg_i)
%
...
% runLifeBothAlgos %
%%%%%%%%%%%%%%%%%%%%
function [stt tend] = runLifeBothAlgos(tend,stt,dwi,nifti,fg_i)
%
...
val.life.w.criterion = 0.015;
val.life.hits        = val.life.w.fiber >= val.life.w.criterion; % the fibers with large weights
