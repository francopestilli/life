function s_life_fg_generate_prediction_nifti(runType,fitType,save)
% function s_life_fg_generate_prediction_nifti
%
% Loads some tracks produced with an algorithm (MRTRIX, STT, or TEND). 
% 
% Produces the quality of fit for the tracks, before and after fitting with LiFE.
%
% Saves out a nifti image with the predicted signal from the life and
% original 
%
...
if notDefined('which_fascicles');which_fascicles = 8;end % There are 20 fascicles
...
saveDir  = fullfile(baseDir,'LiFE','results_nifti');
...
for fg_i = 1:length(which_fascicles)
  fprintf('\n[%s] Running tests on fascicle #%i/%i...\n',mfilename,fg_i,length(which_fascicles));
  
  % load the fibers
  fe = loadFiberGroupLocal(fe,which_fascicles(fg_i));
  fprintf('\n[%s] Analyzing Fascicle %s\n',mfilename,fe.fg.img.name)

  % Run LiFE
  fe = mctRunLifeFitsLocal(fe,dwi);
    
  % Now generate corss-validate quality fo fit
...
switch fe.algo
  case {'MRTRIX'}
    fe.fg.name        = fullfile(sprintf('%s_%s',fiberDir,fe.algo),sprintf('MoriGroups_clean_D4_L3_%s.mat',fe.algo));
  otherwise
    fe.fg.name        = fullfile(sprintf('%s_%s',fiberDir,fe.algo),sprintf('MoriGroups_clean_D5_L4_%s.mat',fe.algo));
end
...
% loadFiberGroupLocal %
%%%%%%%%%%%%%%%%%%%%%%%
function fe = loadFiberGroupLocal(fe,fg_i)
%
% Load the contrack fibers and the MRTRIX, STT or TEND fibers.
%
fprintf('\n[%s]\n loading fiber from file: %s\n',mfilename,fe.fg.name)
temp_fg = dtiLoadFiberGroup(fe.fg.name);
fe.fg.acpc = temp_fg(fg_i);clear temp_fg;
