function feFileToLoad = s_ms_test_connectomes_signal_prediction_dig_voxels(connectomeType,rep)
%
% feFileToSave = s_ms_test_connectomes_signal_prediction_dig_voxels(connectomeType,rep)
%
% This script is a copy of s_ms_test_connectomes_signal_prediction.m
% It provides two additional plots. Showing how the LIFE signal
% predictions, compare to the Data as function of the numbr of nodes in a
% voxel. 
%
% The main result is that voxel with small number of nodes tend to have a
% LIFE predicted signal close to zero.
%
...
if notDefined('percentiles'), percentiles = {[0 100]};end%{[20 80],[10,90],[0 100],[0 50],[50 100],[0:5:100],[0 .25 .5 1 2 4 8 16 32 64 100]};end
...
for icull = 1:length(cullType)
    for ip = 1:length(percentiles)
        ...
        for irep = 1:length(rep)
            ...
            for iPrctl = 1:length(percentiles{ip})-1
                ...
                for ivx = 1:length(goodvoxels)
                    vxrows{ivx} = feGet(fe,'voxelsindices',{goodvoxels(ivx)});
                end
                ...
                [~, DFMw, R2] = feFitModel(DFM,Data1,'sgdnn');
                DFM_psig      = full(DFM * DFMw);
                
                % Count the nomber of nodes per voxel
                for ivx = 1:length(goodvoxels)
                    nnodes(ivx) = sum(fe.life.voxel2FNpair{goodvoxels(ivx)}(:,2));    
                end
                
                % Find the sum of the predicted signal in each voxel         
                nBvecs  = feGet(fe,'nBvecs');
                nVoxels = feGet(fe,'n voxels');
                LIFE_psig_vx = reshape(feGet(fe, 'psigfiber'), nBvecs, nVoxels);
                Data1_vx = reshape(feGet(fe, 'dsigdemeaned'), nBvecs, nVoxels);
                for ivx = 1:length(goodvoxels)
                    % Get the signal from this voxel
                    LIFE_psig_tmp{ivx} = (LIFE_psig_vx(:,goodvoxels(ivx)));
                    Data1_sig_tmp{ivx} = (Data1_vx(:,goodvoxels(ivx)));
                end
                
                % plot the number of nodes with the sum of predicted signal
                % Find the voxels with smal number of nodes
                idx = nnodes <= 50;
                figure;plot(cell2mat(Data1_sig_tmp(idx)),cell2mat(LIFE_psig_tmp(idx)),'ro')
                ylabel('LIFE prediction','Fontsize',16)
                xlabel('Measured diffusion signal','Fontsize',16)
                axis([-300 300 -300 300])
                title('Voxels with LESS than 50 nodes','Fontsize',16)
                ...
                xlabel('Measured diffusion signal','Fontsize',16)
                axis([-300 300 -300 300])
                title('Voxels with MORE than 50 nodes','Fontsize',16)

                % % This is a test to show that the stochastic gradient
                % % descent and the the full fit return quais-identical
                % % rsutls. It works.
                % Psedo inverse solution
                %DFMps_w = (DFM' * DFM).^-1 * DFM' * Data1;
                %DFMps_psig = full(DFM * DFMps_w);
                % NB the following lines take very long time to execute ad
                % have not yet seen an end to the process... but this is
                % the way it *could* be fit.
                %LIFEps = feGet(fe, 'Mfiber',1:600);
                %Data1ps     = feGet(fe, 'dsigdemeaned',1:600);
                %LIFEps_w = (LIFEps' * LIFEps).^-1 * LIFEps' * Data1ps;
    ...
    figName = sprintf('Prediction_improvement_Rrmse_%s_%s',fName,cullType{icull});
    x_label = 'R_{rmse}';
