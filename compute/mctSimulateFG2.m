function [fgHit fgFa dwi nifti hitRoiCoords faRoiCoords] = mctSimulateFG2(fiberType,bval, nbvecs, S0,ns,mmPerNode,numClones,clones_sd)
%
% function [fgHit fgFA dwi nifti hitRoiCoords faRoiCoords] = mctSimulateFG2(fiberType,bvals, bvecs,S0,ns,mmPerNode,numClones,clones_sd)
%
...
%     nFiber = 2;
%     ns     = 0.125
%     [fg dwi nifti coords] = mctSimulateFG2(fiberType, bval, nbvecs, S0,ns);
%
...
if numClones > 0;
    fgHit = fgClone(fgHit,numClones,clones_sd);
    fgFa  = fgClone(fgFa,numClones,clones_sd);
end
...
hitRoiCoords = bsxfun(@max,fefgGet(fgHit,'unique image coords'),1);
faRoiCoords  = bsxfun(@max,fefgGet(fgFa,'unique image coords'),1);
...
for ff = 1:length(fg.fibers)
  ...
  if size(this_f,1) == 3;
    intFiber = mctInterpFiber(mmPerNode,this_f')';
  elseif size(this_f,2) == 3;
    intFiber = mctInterpFiber(mmPerNode,this_f);
  else
    keyboard
  end
  fg.fibers{ff} = intFiber;

% Old way of interpolating linearly along the x,y and z directions
%
%     y  = fg.fibers{ff};
%     x  = 1:size(y,2);    
...
% fgClone %
%%%%%%%%%%%
function fg = fgClone(fg,numClones,clones_sd)
...
if notDefined('clones_sd'), clones_sd=5.95; end
fgTemp = fg;
...
for ii = 1:length(fg.fibers)
  this_f = fg.fibers{ii};
  for ic = 1:numClones
    % generate the jiter to add to eahc x,y,z coord for cloning.
    jitter = randn( size(this_f) ) .* clones_sd;
    
    % Clip to 3 standard deviations. 
    %jitter(jitter > clones_sd*3) = clones_sd*3;
    
    fgTemp.fibers{count} = this_f + jitter;
