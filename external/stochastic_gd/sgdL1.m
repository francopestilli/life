function [w, R2] = sgdL1(y,X,numtoselect,finalstepsize,convergencecriterion,checkerror,nonneg,alpha,lambda)
% 
...
% 
% sgdL1(y,X,numtoselect,finalstepsize,convergencecriterion,checkerror,nonneg)
% 
...
if notDefined('alpha'),  alpha  = 0.9; end 
...
while 1
  ...
  if mod(iter,checkerror) == 1
    ...
    est_ssq = sum((y - X*w).^2) + lambda*(alpha*sum(abs(w)) + (1-alpha)*sum(w.^2));
